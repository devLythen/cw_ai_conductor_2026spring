...
static void my_application_activate(GApplication* application) {
  ...
  // Frameless window — no system title bar.
  gtk_window_set_decorated(window, FALSE);
  gtk_window_set_default_size(window, 1280, 720);
  gtk_widget_set_size_request(GTK_WIDGET(window), 1050, 600);
  ...
  // Set up window-control method channel for frameless window.
  g_autoptr(FlStandardMethodCodec) codec = fl_standard_method_codec_new();
  FlMethodChannel* wc = fl_method_channel_new(
      fl_engine_get_binary_messenger(fl_view_get_engine(view)),
      "winbsod/window",
      FL_METHOD_CODEC(codec));
  fl_method_channel_set_method_call_handler(wc, window_method_call_cb, window,
                                            nullptr);
